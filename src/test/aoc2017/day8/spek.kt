...
class Test : Spek({
    ...
    given("1st task") {
        on("a single instruction line") {
            val str = "av inc 167 if f > -9"
            val instr = Instruction.parse(str)

            it("'s reg is `av`") {
                assertEquals("av", instr.reg)
            }

            it("is increasing") {
                assertTrue(instr.inc)
            }

            it("'s number is 167") {
                assertEquals(167, instr.num)
            }

            it("'s condition register is `f`") {
                assertEquals("f", instr.condReg)
            }

            it("'s condition operation is `>`") {
                assertEquals(ComparisonOperator.GT, instr.condCmp)
            }

            it("'s condition number is -9") {
                assertEquals(-9, instr.condNum)
            }
        }
