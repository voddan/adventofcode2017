...
fun TestContainer.itAsserts(input: String, expect: List<Region>) {
    it("parses ${expect.count {it is Garbage}} garbage region(s) " +
            "and ${expect.count {it is Group}} group(s) in `$input`") {
        assertEquals(expect, parse(input))
    }
}
